...
# Third-party sources pinned in configs/sources.lock are fetched and symlinked into an
# aggregate feed dir alongside the first-party package/ dirs, so "our packages" remain a single
# h5000m feed (one signed index, one offline-repo harvest). With an empty sources.lock this is a
# no-op and we link package/ directly, keeping the proven path byte-for-byte.
"${ROOT_DIR}/scripts/fetch-sources.sh"
sources_nonblank="$(sed -e 's/#.*//' -e '/^[[:space:]]*$/d' "${ROOT_DIR}/configs/sources.lock" || true)"
if [ -n "${sources_nonblank}" ]; then
  abs_pkg="${ROOT_DIR}/.sources/h5000m-feed"
  echo ">> sources.lock non-empty: h5000m feed = aggregate (first-party + third-party)"
else
...
# Third-party packages named in configs/sources.lock `builds=` are selected the same way.
mapfile -t SRC_PKGS < <(sed -n 's/^[[:space:]]*builds[[:space:]]*=[[:space:]]*//p' \
  "${ROOT_DIR}/configs/sources.lock" | tr ' ' '\n' | sed '/^[[:space:]]*$/d')
ALL_PKGS=( "${BUILD_PKGS[@]}" "${SRC_PKGS[@]}" )
EXPECTED_COUNT="${#ALL_PKGS[@]}"
...
for pkg in "${ALL_PKGS[@]}"; do
  grep -qx "CONFIG_PACKAGE_${pkg}=m" "${SDK}/.config" \
    || fail "package not selected =m after defconfig: ${pkg}"
  selected=$((selected + 1))
done
